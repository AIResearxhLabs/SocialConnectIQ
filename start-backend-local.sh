...
# =============================================================================
# Backend Services Startup Script - LOCAL MCP MODE
# =============================================================================
# This script starts all backend microservices configured for LOCAL Docker MCP
# MCP Server: http://localhost:3001

set -e

echo "🚀 Starting Social Media Management Backend Services (LOCAL MCP MODE)..."
echo ""

# Export LOCAL MCP configuration
export MCP_HOST_TYPE=local
export MCP_LOCAL_URL=http://localhost:3001
export MCP_SERVER_URL=http://localhost:3001  # Fallback

echo "📍 MCP Configuration: LOCAL (Docker Desktop)"
echo "   MCP URL: ${MCP_LOCAL_URL}"
echo ""
...
if command -v curl &> /dev/null; then
    if curl -s --connect-timeout 5 "${MCP_LOCAL_URL}/health" > /dev/null 2>&1 || \
       curl -s --connect-timeout 5 "${MCP_LOCAL_URL}/mcp/tools" > /dev/null 2>&1; then
        echo "   ✅ Local MCP server is running and accessible"
    else
        echo "   ⚠️  WARNING: Cannot reach local MCP server at ${MCP_LOCAL_URL}"
        echo ""
        echo "   Make sure your MCP Docker container is running:"
        echo "   → docker ps | grep mcp"
        echo "   → docker start mcpsocial"
        echo ""
...
echo ""
echo "✅ All services started in LOCAL MCP MODE!"
echo ""
echo "📊 Service Status:"
...
echo ""
echo "📍 MCP Server:          http://localhost:3001 (LOCAL Docker)"
echo ""
echo "📝 Logs are available in: services/logs/"
echo "🛑 To stop services, run: ./stop-backend-local.sh"
echo ""
