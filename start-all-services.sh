...
# Starts: API Gateway + Backend Service + Integration Service + Scheduling Service + Agent Service
...
kill_port 8003  # Scheduling Service
...
# Start Scheduling Service (Port 8003)
echo "=================================="
echo "📅 Starting Scheduling Service (Port 8003)"
echo "=================================="
...
echo "🚀 Launching Scheduling Service..."
nohup python3 -m uvicorn app.main:app --host 127.0.0.1 --port 8003 --reload > ../../logs/scheduling-service.log 2>&1 &
SCHEDULING_PID=$!
echo $SCHEDULING_PID > ../pids/scheduling-service.pid
echo "✅ Scheduling Service started (PID: $SCHEDULING_PID)"
cd ../..
echo ""

# Wait for Scheduling Service
echo "⏳ Waiting for Scheduling Service..."
for i in {1..30}; do
    if check_port 8003; then
        echo "✅ Scheduling Service is ready!"
        break
    fi
    if [ $i -eq 30 ]; then
        echo "❌ Scheduling Service failed to start"
        exit 1
    fi
    sleep 1
done
...
echo "  📅 Scheduling Service:  http://localhost:8003  (PID: $SCHEDULING_PID)"
...
echo "  📅 Scheduling Docs:     http://localhost:8003/docs"
...
echo "  📅 Scheduling:          http://localhost:8003/health"
...
echo "  📅 Scheduling Service:  logs/scheduling-service.log"
