...
async def disconnect_linkedin(user_id: str = Header(..., alias="X-User-ID")):
    ...
    try:
        user_ref = db.collection('users').document(user_id)
        user_ref.update({
            'integrations.linkedin': firestore.DELETE_FIELD
        })
...
async def twitter_auth(user_id: str = Header(..., alias="X-User-ID")):
    """Initiate Twitter OAuth flow"""
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"{MCP_SOCIAL_URL}/tools/getTwitterAuthUrl/run",
                json={"userId": user_id}
            )
            response.raise_for_status()
            data = response.json()
            ...
            raise HTTPException(status_code=500, detail=f"Failed to get Twitter auth URL: {e}")
...
async def twitter_callback(code: str, state: Optional[str] = None):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{MCP_SOCIAL_URL}/tools/handleTwitterAuthCallback/run",
                json={"code": code, "userId": user_id}
            )
            response.raise_for_status()
            token_data = response.json()
            
            await save_user_tokens(user_id, 'twitter', token_data)
            
            return RedirectResponse(url=f"http://localhost:3000/oauth-callback.html?status=success&platform=twitter")
            
    except Exception:
        return RedirectResponse(url=f"http://localhost:3000/oauth-callback.html?status=error&platform=twitter")
